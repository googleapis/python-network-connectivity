...
class PolicyBasedRoutingServiceClient(metaclass=PolicyBasedRoutingServiceClientMeta):
    ...
    @staticmethod
    def policy_based_route_path(project: str,location: str,policy_based_route: str,) -> str:
        """Returns a fully-qualified policy_based_route string."""
        return "projects/{project}/{location}/global/PolicyBasedRoutes/{policy_based_route}".format(project=project, location=location, policy_based_route=policy_based_route, )

    @staticmethod
    def parse_policy_based_route_path(path: str) -> Dict[str,str]:
        """Parses a policy_based_route path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/(?P<location>.+?)/global/PolicyBasedRoutes/(?P<policy_based_route>.+?)$", path)
        return m.groupdict() if m else {}
